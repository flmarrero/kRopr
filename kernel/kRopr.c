...
static int device_open(struct inode* inode, struct file* file) {
        printk(KERN_INFO "kRopr: Opened device\n");
...
static int device_release(struct inode* inode, struct file* file) {
        printk(KERN_INFO "kRopr: Device released!\n");

        return 0;
}

static int m_init(void) {
        printk(KERN_INFO "kRopr: Starting kRopr...\n");
        printk(KERN_INFO "kRopr: addr(ops) = %p\n", &ops);
        printk(KERN_INFO "kRopr: addr(fops) = %p\n", &fops);
...
static void m_exit(void) {
        ...
        printk(KERN_INFO "kRopr: Stopped kRopr!\n");
}
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("This driver is a dummy driver for testing and developing kRopr");
